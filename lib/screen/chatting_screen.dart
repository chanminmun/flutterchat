...
class _ChattingScreenState extends State<ChattingScreen> {
  ...
  Future<void> _sendLikeOrDislikeMessage(
    ...
    final user = _auth.currentUser;
    if (user != null) {
      String color = action == '좋아요를 눌렀습니다' ? '#008000' : '#FF0000'; // 좋아요는 green, 싫어요는 red
      await _firestore.collection('chats').add({
        'text': '$originalUserName님이 보낸 "$originalMessage" 메시지에 대해 $_userName님은 $action!',
        'createdAt': Timestamp.now(),
        'username': _userName,
        'userId': user.uid,
        'color': color,
                                      ...
                                      IconButton(
                                        ...
                                        onPressed: () async {
                                          await _sendLikeOrDislikeMessage(
                                              '좋아요를 눌렀습니다', chatData['text'], username);
                                        },
                                      ),
                                      IconButton(
                                        ...
                                        onPressed: () async {
                                          await _sendLikeOrDislikeMessage(
                                              '싫어요를 눌렀습니다', chatData['text'], username); // 싫어요 클릭 시 메시지 전송
                                        },
                                      ),
                                    ],
                                  ),
