      ...
      if (googleUser != null) {
        ...
        if (user != null) {
          ...
          String userProfilePicUrl = user.photoURL ??
              'https://example.com/default_profile_pic.png'; // 기본 프로필 사진 URL
      ...
      for (var userDoc in querySnapshot.docs) {
        ...
        if (locationSnapshot.docs.isNotEmpty) {
          ...
          double distance = _calculateDistance(
              userLat, userLng, geoPoint.latitude, geoPoint.longitude);
  ...
  Future<void> _logout() async {
    try {
      ...
      if (firebaseUser != null) {
        // 위치 정보 삭제
        await _deleteUserLocation(firebaseUser.uid);
        print('위치 정보 삭제 완료');
      }
...
// Firestore에서 위치 데이터 삭제
  Future<void> _deleteUserLocation(String userId) async {
    try {
      // Firestore에서 'users' 컬렉션 안에 있는 해당 사용자 UID로 위치 정보 삭제
      var locationsCollection = FirebaseFirestore.instance
          .collection('users')
          .doc(userId)
          .collection('locations');

      // 위치 정보 삭제 (모든 문서 삭제)
      var snapshot = await locationsCollection.get();
      for (var doc in snapshot.docs) {
        await doc.reference.delete();
      }

      print('사용자 위치 정보 삭제 완료');
    } catch (e) {
      print('위치 정보 삭제 중 오류 발생: $e');
    }
  }
            ...
            child: Column(
              ...
              children: [
                if (_nearbyUsers.isEmpty)
                  ...
                  // 근처 사용자 목록 표시
                  ListView.builder(
                    shrinkWrap: true, // 리스트의 크기를 부모의 크기에 맞추기
                    physics: NeverScrollableScrollPhysics(), // ListView가 스크롤되지 않도록 설정
      ...
      appBar: AppBar(
        ...
        leading: _userProfilePicUrl.isEmpty
          ...
          child: const Icon(
            ...
            : GestureDetector(
          onTap: () {
            // 프로필 사진이 있을 경우 홈 화면으로 이동
            setState(() {
              _selectedIndex = 0; // 홈 화면으로 전환
            });
          },
          child: const Icon(
            Icons.home, // 홈 아이콘으로 변경
            color: Colors.white,
            size: 30,
          ),
        ), // 프로필이 있을 경우 아이콘을 홈 아이콘으로 설정
        title: Row(
